...
import { child, getDatabase, onChildChanged, onValue, query, ref, set, update } from 'firebase/database';
  ...
  const fetchCommunityLiveSessions = () => {
    ...
    return onValue(child(documentRef, `liveClass`), (snapshot) => {
      if (snapshot.exists()) {
        ...
        const filteredSessions = Object.keys(data).filter((obj) => {
          return data[obj]?.communitySession === true;
        });
